...
import com.wingedsheep.mtg.sets.definitions.lrw.cards.BoggartHarbinger
...
class BoggartHarbingerScenarioTest : FunSpec({
    fun driver(): GameTestDriver = GameTestDriver().apply {
        registerCards(TestCards.all + listOf(BoggartHarbinger))
        initMirrorMatch(deck = Deck.of("Plains" to 40), startingPlayer = 0)
        passPriorityUntil(Step.PRECOMBAT_MAIN)
    }

    fun GameTestDriver.castHarbinger() {
        val source = putCardInHand(player1, "Boggart Harbinger")
    ...
    test("declining the search preserves library order and emits no search or shuffle") {
        val d = driver()
        d.putCardOnTopOfLibrary(d.player1, "Boggart Harbinger")
        d.castHarbinger()
    ...
    test("accepting the search reveals the chosen tribal card and puts it on top") {
        val d = driver()
        val found = d.putCardOnTopOfLibrary(d.player1, "Boggart Harbinger")
