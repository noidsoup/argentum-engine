 ...
 * `YouCast(Any.withColor(GREEN))` + `ReduceGeneric(1)`. `CardPredicate.HasColor` is evaluated
 * against the spell's *colors*, not its color identity, so a multicolored spell that is partly
 * green is a green spell and gets the discount. `ReduceGeneric` only shaves generic mana, which
 * is exactly what the rulings require.
 */
val EmeraldMedallion = card("Emerald Medallion") {
    manaCost = "{2}"
    colorIdentity = ""
    typeLine = "Artifact"
    ...
    metadata {
        ...
        imageUri = "https://cards.scryfall.io/normal/front/6/7/67e87f30-b27a-48e2-a133-192309dd5902.jpg?1783946605"
