...
engine. Sits between [`:gym`](../gym/README.md) (the env
...
`:gym` is transport-agnostic and decision-aware: it knows how to
...
`:gym-trainer` is where the RL-shaped opinions live:
...
[`:gym-server`](../gym-server/README.md). If your trainer
...
is just a state advance), use [`:gym-server`](../gym-server/README.md)'s
...
The library only depends on `:gym`, `:rules-engine`, `:mtg-sdk`
...
- **No multi-process worker pool.** `:gym`'s `MultiEnvService.stepBatch`
...
./gradlew :gym-trainer:test # same via gradle
```
