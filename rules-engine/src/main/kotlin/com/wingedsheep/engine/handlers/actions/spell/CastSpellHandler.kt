...
class CastSpellHandler(
    ...
    override fun validate(state: GameState, action: CastSpell): String? {
        ...
        val mayPlayFromHand = !inHand && !onTopOfLibrary && !mayPlayFromExile &&
            zoneResolver.isInHandWithMayPlayPermission(state, action.playerId, action.cardId)
        val mayCastFromZone = !inHand && !onTopOfLibrary && !mayPlayFromExile && !mayPlayFromHand &&
            zoneResolver.hasMayCastSelfFromZonePermission(state, action.playerId, action.cardId)
        val mayCastFromGraveyard = !inHand && !onTopOfLibrary && !mayPlayFromExile && !mayPlayFromHand && !mayCastFromZone &&
        ...
        if (!inHand && !onTopOfLibrary && !mayPlayFromExile && !mayPlayFromHand && !mayCastFromZone && !mayCastFromGraveyard && !hasFlashback && !hasHarmonize && !hasMayhem && !hasGraveyardCast && !hasForageFromGraveyard && !hasWarpFromGraveyard && !hasCommanderCast && !hasGraveyardSneak && disturbFace == null) {
